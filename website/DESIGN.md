...
- In the capture diagram, a thin trace follows the two real handoffs: agent hooks to why-diff, then evidence to local Git. In the checkpoint diagram, the before, tool, and after cards highlight in order. Keep both effects quiet, with no glowing lights or arrowheads, and show static labels when motion is reduced.
...
- The footer keeps “Always free and open source” beside the brand on the same text baseline; Docs and GitHub stay as separate links. The header remains visible while scrolling, with anchor targets offset below it.
